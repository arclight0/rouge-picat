...
run do |env|
  ...
  Object.send(:remove_const, :RougePicat) if defined?(::RougePicat)
  ...
  # load the info
  input = File.read(ENV["SAMPLE_FILE"] || "#{__dir__}/spec/sample.pi")

  # use the query string for lexer options
  lexer = Rouge::Lexer.find_fancy("picat?#{env['QUERY_STRING']}")
